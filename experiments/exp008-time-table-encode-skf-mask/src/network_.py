...
class TimeEncoder(nn.Module):
    ...
    def forward(self, active_logs, active_mask):
        ...
        embedded, attention_weights = self.multihead_attention(embedded, active_mask.to(torch.float)) # (1, 31, 320) # (batch, day, time*channel)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        x, attention_weights = self.attention(query=x, key=x, value=x, attn_mask=mask.unsqueeze(-1).repeat(4, 1, 31))
        return x, attention_weights