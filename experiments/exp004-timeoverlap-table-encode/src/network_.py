...
class CMIModel(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, dropout=0.0):
        ...
        self.linear = nn.Linear(32+320, 1)
...
class TimeEncoder(nn.Module):
    ...
    def forward(self, active_logs, active_mask):
        ...
        timeseries = timeseries.squeeze(0) # (32, 31, 10) # (channel, day, time)
        
        embedded = []
        for day in range(31):
            # timeseries[:, day, :] = timeseries[:, day, :]#  * active_mask[day]
            day_embedded, _ = self.encoder(timeseries[:, day, :].unsqueeze(0).permute(0, 2, 1)) # x: (seq_len, batch, input_size), _: (h_n, c_n)
            embedded.append(day_embedded)

        embedded = torch.cat(embedded, dim=0).unsqueeze(0) # (1, 31, 10, 32) # (batch, day, time, channel)
        embedded, attention_weights = self.multihead_attention(embedded) # (1, 31, 320) # (batch, day, time*channel)
        
        pooled_embedded = embedded * active_mask.unsqueeze(-1) # (1, 31, 320) # (batch, day, time*channel)
        pooled_embedded = torch.sum(pooled_embedded, dim=1) # (1, 320) # (batch, time*channel)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        x, attention_weights = self.attention(query=x, key=x, value=x, attn_mask=mask)
        return x, attention_weights