...
class CMIModel(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, dropout=0.0):
        ...
        self.query_encoder = nn.Sequential(
            nn.Linear(table_input_size, 320)
        )
    ...
    def forward(self, table_input, time_input, active_mask):
        ...
        query_encoded = self.query_encoder(table_input)
        pooled_embedded, embedded, attention_weights = self.timeencoder(time_input, active_mask, query_encoded)
...
class TimeEncoder(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, dropout=0.0):
        ...
        self.subsample = nn.Sequential(
            ...
            nn.Conv1d(in_channels=15, out_channels=32, kernel_size=kernel_size, stride=stride, padding=padding),
            nn.ReLU(),
            nn.LayerNorm([32, 44640]),
            nn.Conv1d(in_channels=32, out_channels=32, kernel_size=kernel_size, stride=stride, padding=padding),
            nn.ReLU(),
    ...
    def forward(self, active_logs, active_mask, query):
        ...
        embedded, attention_weights = self.multihead_attention(embedded, active_mask.to(torch.float), query) # (1, 31, 320) # (batch, day, time*channel)
