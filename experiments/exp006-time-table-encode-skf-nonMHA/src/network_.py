...
class CMIModel(nn.Module):
    ...
    def forward(self, table_input, time_input, active_mask):
        pooled_embedded, embedded = self.timeencoder(time_input, active_mask)
...
class TimeEncoder(nn.Module):
    ...
    def forward(self, active_logs, active_mask):
        ...
        embedded, _ = self.encoder(timeseries) # x: (batch, day*time, input_size), _: (h_n, c_n)

        # embedded, attention_weights = self.multihead_attention(embedded) # (1, 31, 320) # (batch, day, time*channel)
        embedded = embedded.view(1, 31, -1) # (1, 31, 320) # (batch, day, time*channel)
